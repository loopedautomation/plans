...
- [ ] Decide the grammar (prefix lines vs. markers) and write it down in
      `html-view.ts` beside `COMMENT`, with a `suggestion(value)` parser
      that returns `{ who, old, new }` or null and never matches a plain
      thread
- [ ] Target resolution: nearest preceding node whose normalised text
      equals `old`; a null target is the stale state
- [ ] The suggestion card as a widget over the hidden run
...
- [ ] Accept and reject through `htmlBridge.apply` (`Editor.tsx:437-447`)
      as one transaction; verify it merges cleanly in a room with a second
      client mid-edit
- [ ] `SUGGEST_PROMPT` in `agent.ts` beside `REWRITE_PROMPT`, its setting
      and Settings textarea; "Suggest a rewrite…" on the page menu reusing
      `rewriteSelection`'s flush-and-quote path
- [ ] "Suggest…" for people: selection → `TextPrompt` → block written after
      the selection's node, signed by the comment system's identity rule
- [ ] Share page: render the card read-only, or decide to hide it
- [ ] Tests: e2e in `e2e/workspace.spec.ts` where an agent's write lands as
